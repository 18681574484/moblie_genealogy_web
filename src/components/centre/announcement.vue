<template>
  <div class="context">
    ...
    <div class="card-1">
      ...
      <router-link
        :to="{ name: 'detailsNotice', params: { id: cement.length ? cement[0].id : '' }}"
        class="card-box card-top"
        tag="div"
        v-html="cement.length ? cement[0].newsText : ''"
      ></router-link>
    </div>
    <!-- 联谊概况 -->
    ...
    <div class="card-1 card-3">
      ...
      <div class="card-box" style="display: block;">
        <p>{{brief.description.slice(0,200)}}</p>
        <div class="box-left">
          <span>
            族谱总数:
    ...
    <div class="fellowship" v-for="item in  cement" :key="item.id">
      <div class="fellowship-header">
        <span>联谊会公告</span>
        <router-link
          :to="{ name: 'detailsNotice', params: { id: item.id }}"
          tag="span"
...
export default {
  ...
  methods: {
    announcement() {
      let bulletin = this.$store.state.apiList;
      this.api
        .get(this.api.county.base + bulletin.index_family_record2.apiUrl)
        .then(res => {
          if (res.code == 200) {
            this.cement = res.data.records;
          }
          return this.api.get(
            this.api.county.base + bulletin.index_summary.apiUrl
          );
        })
        .then(res => {
          if (res.code == 200) {
          ...
          return this.api.get(
            this.api.county.base + bulletin.index_fan_summary.apiUrl
          );
        })
        .then(res => {
          if (res.code == 200) {
